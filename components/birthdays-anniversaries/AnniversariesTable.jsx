...
import { Phone, Eye, MoreVertical, ChevronLeft, ChevronRight, Heart } from "lucide-react";
import { AnniversaryDetailsModal } from "./AnniversaryDetailsModal";


export function AnniversariesTable({ title, todaysDate, records = [] }) {
  ...
  return (
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-full text-left text-sm">
          <thead className="border-y border-border bg-surface-canvas text-xs uppercase tracking-wide text-ink-subtle">
            <tr>
              {["Date", "Couple", "Years Married", "Location / Branch", "Contact", "Remarks", "Actions"].map((h) => (
              ...
              <tr key={record.id}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-accent-50 text-accent-500">
                      <Heart className="h-4 w-4" />
                    </div>
                    <p className="font-semibold text-ink">{record.names}</p>
                  </div>
                </td>
                <td className="px-4 py-3 text-ink">{record.yearsMarried} years</td>
    ...
    <AnniversaryDetailsModal record={selectedRecord} open={modalOpen} onOpenChange={setModalOpen} />
    </>
  );
}
