...
import { YouthGroupRowActionsMenu } from "./YouthGroupRowActionsMenu";
import { YG_STATUS_VARIANT, YG_STATUS_OPTIONS } from "@/lib/mock/youthGroupsMockData";

export function YouthGroupsTable({
  groups, isLoading, pagination,
  search, onSearchChange, statusFilter, onStatusFilterChange,
  onEdit, onManageMembers, onManageLeaders, onGroupAttendance, onDeactivate, onDelete,
}) {
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <Link href={`/youth-ministry/youth-groups/${row.id}`} className="font-medium text-ink hover:text-interactive-600 hover:underline">
              {row.name}
            </Link>
            <p className="text-xs text-ink-subtle">Ages {row.ageLabel}</p>
          </div>
        </div>
      ),
    },
    ...
    { key: "meeting", header: "Meeting Day & Time", render: (row) => <span className="text-ink-muted">{row.meetingDay}, {row.meetingTime}</span> },
    {
      key: "status", header: "Status",
      render: (row) => <Badge variant={YG_STATUS_VARIANT[row.status] ?? "default"}>{row.status}</Badge>,
    },
    {
      key: "actions", header: "Actions", cellClassName: "text-right", className: "text-right",
      render: (row) => (
        <div className="flex items-center justify-end gap-1.5">
          <Link href={`/youth-ministry/youth-groups/${row.id}`} className="flex h-8 w-8 items-center justify-center rounded-md border border-border text-ink-subtle hover:bg-surface-canvas" aria-label={`View ${row.name}`}>
          ...
          <YouthGroupRowActionsMenu
            group={row}
            onView={() => {}} onEdit={onEdit} onManageMembers={onManageMembers} onManageLeaders={onManageLeaders}
            onGroupAttendance={onGroupAttendance} onDeactivate={onDeactivate} onDelete={onDelete}
          />
        </div>
      ),
    },
  ...
  return (
    <div className="rounded-lg border border-border bg-white shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
        <h3 className="text-base font-semibold text-ink">Youth Groups List</h3>
        <div className="flex items-center gap-2">
          <select value={statusFilter} onChange={(e) => onStatusFilterChange(e.target.value)} className="h-9 rounded-md border border-border bg-white px-3 text-sm text-ink-muted">
            ...
            {YG_STATUS_OPTIONS.map((s) => <option key={s}>{s}</option>)}
          </select>
