...
export function AttendanceByMinistryCard({ total, breakdown = [] }) {
  return (
    <div className="rounded-lg border border-border bg-white p-4 shadow-card">
      <h3 className="mb-3 text-sm font-semibold text-ink">Attendance by Ministry</h3>
        ...
        <div className="flex flex-1 flex-col gap-2.5">
          {breakdown.map((entry) => (
            <div key={entry.label} className="flex items-center gap-2 text-sm">
              ...
              <span className="ml-auto font-medium text-ink">{entry.pct}% ({entry.count})</span>
            </div>
          ))}
        </div>
      </div>
