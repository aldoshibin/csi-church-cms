...
const KIND_STYLE = {
  pdf: { bg: "bg-danger-50", color: "text-danger-500", Icon: FileText },
  image: { bg: "bg-interactive-50", color: "text-interactive-600", Icon: ImageIcon },
};

export function ActivityAttachmentsCard({ attachments = [] }) {
  return (
    <div>
      <h3 className="mb-3 text-base font-semibold text-ink">Attachments ({attachments.length})</h3>
      <div className="flex flex-col gap-2.5">
        {attachments.map((file, i) => {
          const style = KIND_STYLE[file.kind] ?? { bg: "bg-surface-muted", color: "text-ink-subtle", Icon: FileText };
