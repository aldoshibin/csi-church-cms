...
export function ActivityAgendaBuilder({ agenda, onAdd, onRemove }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-white p-6 shadow-card">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-base font-semibold text-ink">Agenda (Optional)</h3>
          <p className="mt-0.5 text-sm text-ink-subtle">Add agenda items for this activity.</p>
        </div>
      ...
      {showForm && (
        <div className="mt-4 grid grid-cols-1 gap-3 rounded-lg border border-border bg-surface-canvas p-4 sm:grid-cols-4">
          <div className="sm:col-span-2">
            <Input label="Agenda Item" placeholder="E.g., Registration & Welcome" value={item} onChange={(e) => setItem(e.target.value)} />
          </div>
          <Input label="Start Time" type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} />
          <Input label="Duration" placeholder="E.g., 15 min" value={duration} onChange={(e) => setDuration(e.target.value)} />
      ...
      <div className="mt-4">
        {agenda.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
            ...
            <p className="text-sm text-ink-subtle">Click &quot;Add Agenda Item&quot; to include agenda topics.</p>
          </div>
        ) : (
