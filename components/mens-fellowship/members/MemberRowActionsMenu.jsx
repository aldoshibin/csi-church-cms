...
import { MoreVertical, Eye, Pencil, MessageSquare, Users2, Ban, Trash2 } from "lucide-react";

const itemClass =
  "flex w-full items-center gap-2 rounded px-2.5 py-2 text-sm text-ink-muted outline-none transition-colors hover:bg-surface-canvas hover:text-ink focus:bg-surface-canvas cursor-pointer";

export function MemberRowActionsMenu({ member, onViewDetails, onEdit, onSendMessage, onAddToGroup, onDeactivate, onDelete }) {
  return (
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content align="end" sideOffset={6} className="z-50 w-52 rounded-md border border-border bg-white p-1.5 shadow-elevated animate-fade-in">
          ...
          <DropdownMenu.Item className={itemClass} onSelect={() => onAddToGroup?.(member)}>
            <Users2 className="h-4 w-4" /> Add to Group
          </DropdownMenu.Item>
