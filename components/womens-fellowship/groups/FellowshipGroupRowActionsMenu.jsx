...
export function FellowshipGroupRowActionsMenu({ group, onView, onEdit, onManageMembers, onDuplicate, onDeactivate, onDelete }) {
  return (
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content align="end" sideOffset={6} className="z-50 w-52 rounded-md border border-border bg-white p-1.5 shadow-elevated animate-fade-in">
          ...
          <DropdownMenu.Item className={itemClass} onSelect={() => onDuplicate?.(group)}>
            <Copy className="h-4 w-4" /> Duplicate Group
          </DropdownMenu.Item>
