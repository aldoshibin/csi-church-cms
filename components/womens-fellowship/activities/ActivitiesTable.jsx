...
import { Search, SlidersHorizontal, Eye, Pencil, MoreVertical, Heart, BookOpen, Users, Cross, HandHeart, Star, MoreHorizontal } from "lucide-react";
import { Table } from "@/components/ui/Table";
import { Badge } from "@/components/ui/Badge";
import { ACT_STATUS_VARIANT, ACT_TYPE_OPTIONS, ACT_STATUS_OPTIONS } from "@/lib/mock/fellowshipActivitiesMockData";
  ...
  const columns = [
    {
      ...
      render: (row) => {
        const typeOption = ACT_TYPE_OPTIONS.find((t) => t.value === row.activityType);
    ...
    { key: "activityType", header: "Activity Type", render: (row) => <span className="text-ink-muted">{row.activityType}</span> },
    {
    ...
    { key: "venue", header: "Venue", render: (row) => <span className="text-ink-muted">{row.venue}</span> },
    {
      ...
      render: (row) => <Badge variant={ACT_STATUS_VARIANT[row.status] ?? "default"}>{row.status}</Badge>,
    },
          ...
          <DropdownMenu.Root>
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.Content align="end" sideOffset={6} className="z-50 w-44 rounded-md border border-border bg-white p-1.5 shadow-elevated animate-fade-in">
                <DropdownMenu.Item className={itemClass}>Duplicate Activity</DropdownMenu.Item>
                <DropdownMenu.Item className={itemClass}>Cancel Activity</DropdownMenu.Item>
                <DropdownMenu.Separator className="my-1 h-px bg-border" />
                <DropdownMenu.Item className={`${itemClass} text-danger-600 hover:text-danger-600`}>Delete Activity</DropdownMenu.Item>
  ...
  return (
    <div className="rounded-lg border border-border bg-white shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
        <h3 className="text-base font-semibold text-ink">Activities List</h3>
        <div className="flex items-center gap-2">
          ...
          <select value={statusFilter} onChange={(e) => onStatusFilterChange(e.target.value)} className="h-9 rounded-md border border-border bg-white px-3 text-sm text-ink-muted">
            ...
            {ACT_STATUS_OPTIONS.map((s) => <option key={s}>{s}</option>)}
          </select>
      ...
      <Table columns={columns} data={activities} isLoading={isLoading} getRowId={(row) => row.id} className="rounded-none border-0 shadow-none" pagination={pagination} />
    </div>
  );
}
