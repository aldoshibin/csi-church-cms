...
import { EVT_TYPE_OPTIONS, EVT_VENUE_OPTIONS, EVT_ORGANIZER_OPTIONS } from "@/lib/mock/fellowshipEventsMockData";

export function NewFellowshipEventForm({ form, setField, onAddDocument }) {
    ...
    <div className="flex flex-col gap-5">
      <div className="rounded-lg border border-border bg-white p-6 shadow-card">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          ...
          <Textarea label="Description" rows={1} maxLength={500} placeholder="Enter event description" helperText={`${form.description.length}/500`} value={form.description} onChange={(e) => setField("description", e.target.value)} />
          ...
          <Select label="Venue" required value={form.venue} onChange={(e) => setField("venue", e.target.value)}>
            ...
            {EVT_VENUE_OPTIONS.map((v) => <option key={v}>{v}</option>)}
          </Select>
          ...
          <Select label="Organizer" required value={form.organizer} onChange={(e) => setField("organizer", e.target.value)}>
            ...
            {EVT_ORGANIZER_OPTIONS.map((o) => <option key={o}>{o}</option>)}
          </Select>
          ...
          <Input label="Email" type="email" placeholder="Enter contact email" value={form.email} onChange={(e) => setField("email", e.target.value)} />
        </div>
      </div>

      <div className="rounded-lg border border-border bg-white p-6 shadow-card">
        <h3 className="mb-4 text-base font-semibold text-ink">Event Settings</h3>
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div>
            ...
            <div className="flex h-[42px] items-center gap-6">
              <label className="flex items-center gap-2 text-sm text-ink">
                <input type="radio" name="reg-required" className="h-4 w-4 accent-interactive-500" checked={form.registrationRequired === "Yes"} onChange={() => setField("registrationRequired", "Yes")} />
                Yes
              </label>
              <label className="flex items-center gap-2 text-sm text-ink">
                <input type="radio" name="reg-required" className="h-4 w-4 accent-interactive-500" checked={form.registrationRequired === "No"} onChange={() => setField("registrationRequired", "No")} />
                No
              </label>
            </div>
          </div>
          ...
          <Input label="Total Capacity" type="number" placeholder="Enter total capacity" value={form.totalCapacity} onChange={(e) => setField("totalCapacity", e.target.value)} />
          <div>
      ...
      <div className="rounded-lg border border-border bg-white p-6 shadow-card">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          <Input label="Dress Code (Optional)" placeholder="Enter dress code" value={form.dressCode} onChange={(e) => setField("dressCode", e.target.value)} />
          ...
          <Textarea label="Notes (Optional)" rows={1} maxLength={250} placeholder="Enter any additional notes" helperText={`${form.notes.length}/250`} value={form.notes} onChange={(e) => setField("notes", e.target.value)} />
        </div>
      </div>
