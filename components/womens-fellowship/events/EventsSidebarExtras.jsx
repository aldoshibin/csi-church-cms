...
import { Plus, Tag, Building2, BarChart3 } from "lucide-react";
import { formatDate } from "@/lib/utils";

export function FeUpcomingEventsCard({ events = [] }) {
  return (
        ...
        {events.map((e, i) => (
          <div key={i} className="flex items-start justify-between gap-3 py-3 first:pt-0 last:pb-0">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-ink-subtle">{formatDate(e.date)} · {e.time}</p>
              <p className="mt-0.5 text-xs text-ink-subtle">{e.venue}</p>
            </div>
            <Badge variant="success" className="shrink-0">In {e.daysAway} days</Badge>
          </div>
        ))}
      </div>
    </div>
...
export function EventsByTypeCard({ breakdown = [] }) {
  return (
    <div className="rounded-lg border border-border bg-white p-4 shadow-card">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-ink">Events by Type</h3>
...
const ACTIONS = [
  { label: "Add New Event", icon: Plus, href: "/womens-fellowship/events/add", accent: true },
  { label: "Event Types", icon: Tag, href: "/womens-fellowship/events" },
  { label: "Venue Management", icon: Building2, href: "/womens-fellowship/events" },
  { label: "View Reports", icon: BarChart3, href: "/womens-fellowship/reports" },
];

export function FeQuickActions() {
  return (
    <div className="rounded-lg border border-border bg-white p-4 shadow-card">
