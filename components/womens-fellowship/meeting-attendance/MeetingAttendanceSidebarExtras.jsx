...
export function MtgUpcomingCard({ meetings = [] }) {
  return (
        ...
        {meetings.map((m, i) => (
          <div key={i} className="flex items-start justify-between gap-3 py-3 first:pt-0 last:pb-0">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-ink-subtle">{m.venue}</p>
            </div>
            <Badge variant="success" className="shrink-0">In {m.daysAway} days</Badge>
          </div>
        ))}
      </div>
    </div>
...
export function AttendanceByTypeCard({ breakdown = [] }) {
  return (
    <div className="rounded-lg border border-border bg-white p-4 shadow-card">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-ink">Attendance by Type</h3>
...
const ACTIONS = [
  { label: "Add New Meeting", icon: Plus, href: "/womens-fellowship/meeting-attendance/add", accent: true },
  { label: "Meeting Types", icon: Tag, href: "/womens-fellowship/meeting-attendance" },
  { label: "View Reports", icon: BarChart3, href: "/womens-fellowship/reports" },
  { label: "Export Attendance", icon: Download, href: "/womens-fellowship/meeting-attendance" },
];

export function MtgQuickActions() {
  return (
    <div className="rounded-lg border border-border bg-white p-4 shadow-card">
