...
export function OutreachOverviewCard({ data, title = "Outreach Activities by Category", totalLabel = "Total Activities", formatValue }) {
  if (!data) return null;
  ...
  return (
    <div className="rounded-lg border border-border bg-white p-5 shadow-card">
      ...
      <div className="mt-3 flex items-center gap-4">
        ...
        <div className="flex flex-1 flex-col gap-2 text-sm">
          {data.breakdown.map((entry) => {
            const pct = ((entry.value / data.total) * 100).toFixed(2);
            return (
      ...
      <p className="mt-4 flex items-center justify-between border-t border-border pt-3 text-sm">
        <span className="text-ink-subtle">{totalLabel}</span>
        <span className="font-medium text-ink">{format(data.total)}</span>
      </p>
    </div>
  );
}
