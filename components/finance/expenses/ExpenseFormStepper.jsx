...
import { ADD_EXPENSE_STEPS } from "@/hooks/useAddExpenseForm";

export function ExpenseFormStepper({ currentStep, completedSteps, onStepClick }) {
  return (
    <div className="flex items-center overflow-x-auto rounded-lg border border-border bg-white p-4 shadow-card">
      {ADD_EXPENSE_STEPS.map((step, idx) => {
        const isCompleted = completedSteps.includes(step.key);
        const isCurrent = step.key === currentStep;
        const clickable = (isCompleted || isCurrent) && step.key !== 4;
        return (
          <React.Fragment key={step.key}>
            ...
            {idx < ADD_EXPENSE_STEPS.length - 1 && <div className={cn("mx-4 h-px flex-1", step.key < currentStep ? "bg-interactive-500" : "bg-border")} />}
          </React.Fragment>
        );
      })}
    </div>
