...
const NEXT_ACTIONS = [
  { icon: List, title: "View this income", desc: "See the details of the recorded income." },
  { icon: Printer, title: "Print Receipt", desc: "Print the income receipt for records." },
  { icon: Plus, title: "Add Another Income", desc: "Record another income in the system." },
  { icon: BarChart3, title: "View Income List", desc: "Go to the income list to see all entries." },
];
...
export default function Step4Success({ onAddAnother, onPrint }) {
  ...
  return (
    <div className="flex flex-col items-center rounded-lg border border-border bg-white p-8 text-center shadow-card">
      ...
      <h3 className="text-lg font-semibold text-ink">Income Added Successfully!</h3>
      <p className="mx-auto mt-1 max-w-md text-sm text-ink-subtle">The income has been recorded and saved successfully.</p>
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-3">
        <Link href="/finance/income">
          <Button type="button" variant="secondary" leftIcon={<ArrowLeft className="h-4 w-4" />}>Back to Income List</Button>
        </Link>
        <Button type="button" variant="secondary" leftIcon={<Printer className="h-4 w-4" />} onClick={onPrint}>Print Receipt</Button>
        <Button type="button" variant="secondary" leftIcon={<Plus className="h-4 w-4" />} onClick={onAddAnother}>Add Another Income</Button>
        <Button type="button" leftIcon={<List className="h-4 w-4" />} onClick={() => router.push("/finance/income")}>View Income List</Button>
      </div>
    </div>
  );
}
