...
import { ADD_INCOME_STEPS } from "@/hooks/useAddIncomeForm";

export function IncomeFormStepper({ currentStep, completedSteps, onStepClick }) {
  return (
    <div className="flex items-center overflow-x-auto rounded-lg border border-border bg-white p-4 shadow-card">
      {ADD_INCOME_STEPS.map((step, idx) => {
        ...
        return (
          <React.Fragment key={step.key}>
            ...
            {idx < ADD_INCOME_STEPS.length - 1 && <div className={cn("mx-4 h-px flex-1", step.key < currentStep ? "bg-interactive-500" : "bg-border")} />}
          </React.Fragment>
        );
      })}
    </div>
