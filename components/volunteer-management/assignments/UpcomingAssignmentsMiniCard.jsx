...
export function UpcomingAssignmentsMiniCard({ assignments = [] }) {
  return (
      ...
      <div className="flex flex-col gap-3.5">
        {assignments.map((a, i) => (
          <div key={i} className="flex items-center justify-between gap-3">
            ...
            <span className="shrink-0 rounded-full bg-success-50 px-2.5 py-1 text-xs font-medium text-success-700">{a.relative}</span>
          </div>
        ))}
      </div>
    </div>
