...
export function InactiveStatCards({ mock }) {
  const cards = [
    { label: "Total Inactive Members", value: mock.cards.totalInactive, sublabel: "Members not active", icon: FaUserClock, tone: "bg-brand-700" },
    { label: "Inactive (3 - 6 Months)", value: mock.cards.inactive3to6.value, sublabel: `${mock.cards.inactive3to6.percent}% of total`, icon: FaUsersLine, tone: "bg-success-600" },
      ...
      {cards.map((card) => (
        <div key={card.label} className="rounded-lg border border-border bg-white p-4 shadow-card">
          <div className="flex justify-center items-center gap-3">
            <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-white bg-[#00796B]`}>
