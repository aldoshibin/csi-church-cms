...
import { MINISTRY_GROUP_OPTIONS, MEMBER_CATEGORY_OPTIONS } from "@/utils/constants";

export function InactiveFilterBar({ onSearchChange, onFieldChange, onClear, onApply }) {
  return (
    <section className="rounded-lg border border-border bg-white p-5 shadow-card">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-ink">Last Attended On or Before</label>
          <input type="date" onChange={(e) => onFieldChange("last_attended_before", e.target.value || undefined)} className="h-10 w-full rounded-md border border-border px-3 text-sm text-ink" />
        </div>

        <div>
          <label className="mb-1.5 block text-sm font-medium text-ink">Inactive Since (Min.)</label>
          <select onChange={(e) => onFieldChange("inactive_since_min_months", e.target.value || undefined)} defaultValue="" className="h-10 w-full rounded-md border border-border px-3 text-sm text-ink">
        ...
        <div>
          ...
          <select onChange={(e) => onFieldChange("ministry_group", e.target.value || undefined)} defaultValue="" className="h-10 w-full rounded-md border border-border px-3 text-sm text-ink">
            ...
            {MINISTRY_GROUP_OPTIONS.map((opt) => (
        ...
        <div>
          ...
          <select onChange={(e) => onFieldChange("inactive_severity", e.target.value || undefined)} defaultValue="" className="h-10 w-full rounded-md border border-border px-3 text-sm text-ink">
            <option value="">All</option>
            <option value="INACTIVE">Inactive</option>
            <option value="VERY_INACTIVE">Very Inactive</option>
          </select>
        </div>
      </div>
