...
import { EventRowActionsMenu } from "./EventRowActionsMenu";
import { EVENT_STATUS_VARIANT, EVENT_CATEGORY_OPTIONS, EVENT_STATUS_OPTIONS } from "@/lib/mock/eventsMockData";
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2.5">
          ...
          <Link href={`/sunday-school/events/${row.id}`} className="font-medium text-ink hover:text-interactive-600 hover:underline">
            {row.title}
          </Link>
        </div>
      ),
    ...
    { key: "registrations", header: "Registrations", render: (row) => <span className="text-ink">{row.registered} / {row.capacity}</span> },
    {
      ...
      render: (row) => <Badge variant={EVENT_STATUS_VARIANT[row.status] ?? "default"}>{row.status}</Badge>,
    },
    {
      key: "actions", header: "Actions", cellClassName: "text-right", className: "text-right",
      render: (row) => (
        <div className="flex items-center justify-end gap-1.5">
          <Link href={`/sunday-school/events/${row.id}`} className="flex h-8 w-8 items-center justify-center rounded-md border border-border text-ink-subtle hover:bg-surface-canvas" aria-label={`View ${row.title}`}>
          ...
          <EventRowActionsMenu
            event={row}
            onView={() => {}} onEdit={onEdit} onViewRegistrations={onViewRegistrations} onDuplicate={onDuplicate} onCancel={onCancel} onDelete={onDelete}
          />
        </div>
      ),
    },
