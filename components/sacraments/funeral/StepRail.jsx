...
export const FUNERAL_STEPS = [
  { n: 1, title: "Deceased Details", sub: "Enter deceased information" },
  { n: 2, title: "Death Details", sub: "Enter death information" },
  { n: 3, title: "Funeral Details", sub: "Enter funeral information" },
  { n: 4, title: "Family Details", sub: "Enter family information" },
  { n: 5, title: "Review & Confirm", sub: "Review all details before saving" },
];

export function FuneralStepRail({ step, onJump }) {
  return (
    <div className="w-[220px] shrink-0 space-y-4">
      <div className="rounded-lg border border-border bg-white p-5 shadow-card">
        {FUNERAL_STEPS.map((s, i) => {
          const done = s.n < step;
            ...
            <button key={s.n} type="button" onClick={() => onJump(s.n)} className="block w-full text-left">
              <div className={cn("flex gap-3 rounded-md", active && "bg-interactive-50/60")}>
                <div className="flex flex-col items-center pl-2 pt-2">
                  ...
                  {i < FUNERAL_STEPS.length - 1 && <div className="w-px flex-1 bg-border" style={{ minHeight: 28 }} />}
                </div>
      ...
      <div className="rounded-lg border border-interactive-100 bg-interactive-50 p-4">
        ...
        <p className="text-xs text-interactive-700/80">
          {step === 5
            ? "Please review all information carefully. You can go back and edit if needed before saving the record."
