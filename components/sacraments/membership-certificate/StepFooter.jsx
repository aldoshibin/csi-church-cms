import { ChevronLeft, ChevronRight, Check, Save } from "lucide-react";
import { Button } from "@/components/ui/Button";

export function MCStepFooter({ step, onBack, onNext, onConfirm, onCancel, onSaveDraft }) {
  return (
    <div className="mt-6 flex items-center justify-between border-t border-border pt-5">
      <div>
        {step === 1 ? (
          <Button variant="secondary" onClick={onCancel}>Cancel</Button>
        ) : (
          <Button variant="secondary" leftIcon={<ChevronLeft className="h-4 w-4" />} onClick={onBack}>
            Previous
          </Button>
        )}
      </div>
      <div className="flex gap-2.5">
        <Button variant="secondary" leftIcon={<Save className="h-4 w-4" />} onClick={onSaveDraft}>Save Draft</Button>
        {step === 4 ? (
          <Button variant="primary" leftIcon={<Check className="h-4 w-4" />} onClick={onConfirm}>
            Confirm & Generate Certificate
          </Button>
        ) : (
          <Button variant="primary" rightIcon={<ChevronRight className="h-4 w-4" />} onClick={onNext}>
            Next
          </Button>
        )}
      </div>
    </div>
  );
}
