...
import { Phone, Mail, CalendarDays, User } from "lucide-react";
import { Modal } from "@/components/ui/Modal";
import { Badge } from "@/components/ui/Badge";
import { Button } from "@/components/ui/Button";
import { CHOIR_STATUS_VARIANT } from "@/lib/mock/choirMembersMockData";
...
export function ChoirMemberDetailModal({ open, onOpenChange, member, isLoading, onEdit }) {
  return (
    <Modal
      open={open}
        ...
        <div className="flex flex-col gap-6">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <Badge variant={CHOIR_STATUS_VARIANT[member.status] ?? "default"}>{member.status}</Badge>
              </div>
              <p className="mt-0.5 text-sm text-interactive-600">{member.voicePart} · {member.role}</p>
          ...
          <div className="grid grid-cols-1 gap-x-8 gap-y-5 border-t border-border pt-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="flex flex-col gap-4">
                <Row label="Voice Part" value={member.voicePart} />
                ...
                <div>
                  ...
                  <div className="mt-1"><Badge variant={CHOIR_STATUS_VARIANT[member.status] ?? "default"}>{member.status}</Badge></div>
                </div>
          ...
          {member.notes && (
            <div className="border-t border-border pt-5">
              ...
              <p className="text-sm leading-relaxed text-ink-muted">{member.notes}</p>
            </div>
          )}
        </div>
      )}
