...
export function CampaignOverviewDonutCard({ data }) {
  if (!data) return null;
  return (
    <div className="rounded-lg border border-border bg-white p-4 shadow-card">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-ink">Campaign Overview</h3>
      ...
      <div className="flex items-center gap-4">
        <div className="relative h-[150px] w-[150px] shrink-0">
          ...
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-[11px] text-ink-subtle">{data.totalLabel}</span>
          </div>
        </div>
        <div className="flex flex-1 flex-col gap-2 text-sm">
          {data.breakdown.map((entry) => (
            <div key={entry.label} className="flex items-center gap-2">
              ...
              <span className="ml-auto shrink-0 font-medium text-ink">{entry.count.toLocaleString()} ({entry.pct}%)</span>
            </div>
          ))}
        </div>
      </div>
