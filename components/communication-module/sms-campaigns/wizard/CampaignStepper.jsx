...
import { SMS_CAMPAIGN_STEPS } from "@/hooks/useNewSmsCampaignForm";

export function CampaignStepper({ step, completedSteps, onStepClick }) {
  return (
    <div className="flex items-center justify-center gap-3 rounded-lg border border-border bg-white p-5 shadow-card sm:gap-6">
      {SMS_CAMPAIGN_STEPS.map((s, idx) => {
        ...
        return (
          <div key={s.key} className="flex items-center gap-3 sm:gap-6">
            ...
            {idx < SMS_CAMPAIGN_STEPS.length - 1 && <span className="h-px w-8 shrink-0 bg-border sm:w-16" />}
          </div>
        );
      })}
    </div>
