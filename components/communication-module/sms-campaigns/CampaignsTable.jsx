...
import {
  SMS_CAMPAIGN_STATUS_VARIANT, SMS_CAMPAIGN_TYPE_VARIANT, SMS_CAMPAIGN_STATUS_OPTIONS, SMS_CAMPAIGN_TYPE_OPTIONS,
...
export function CampaignsTable({
  ...
  const columns = [
    ...
    { key: "type", header: "Type", render: (row) => <Badge variant={SMS_CAMPAIGN_TYPE_VARIANT[row.type] ?? "default"}>{row.type}</Badge> },
    {
    ...
    { key: "status", header: "Status", render: (row) => <Badge variant={SMS_CAMPAIGN_STATUS_VARIANT[row.status] ?? "default"}>{row.status}</Badge> },
    {
    ...
    },
    {
      ...
      render: (row) => row.delivered != null ? (
        <div className="flex flex-col gap-1 text-xs">
          <span className="text-ink-subtle">Delivered <span className="font-medium text-ink">{row.delivered.toLocaleString()} ({row.deliveredPct}%)</span></span>
          <span className="text-ink-subtle">Replies <span className="font-medium text-ink">{row.replies.toLocaleString()} ({row.repliesPct}%)</span></span>
        </div>
      ) : <span className="text-ink-subtle">&ndash;</span>,
    },
    {
    ...
    <div className="rounded-lg border border-border bg-white shadow-card">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-border px-4 py-3">
        ...
        <select value={statusFilter} onChange={(e) => onStatusFilterChange(e.target.value)} className="h-9 rounded-md border border-border bg-white px-3 text-sm text-ink-muted">
          ...
          {SMS_CAMPAIGN_STATUS_OPTIONS.map((s) => <option key={s}>{s}</option>)}
        </select>
