...
import { RECENT_VIEWED_BY_FILTER_OPTIONS, RECENT_TYPE_FILTER_OPTIONS } from "@/lib/mock/vmRecentDocumentsMockData";
import { FOLDER_OPTIONS } from "@/lib/mock/vmDocumentManagementMockData";

export function RecentDocumentsFiltersCard({
  ...
  return (
    <div className="rounded-lg border border-border bg-white p-5 shadow-card">
      ...
      <div className="mt-4">
        <label className={labelClass}>Viewed By</label>
        <select value={viewedByFilter} onChange={(e) => setViewedByFilter(e.target.value)} className={selectClass}>
          <option>All Users</option>
          {RECENT_VIEWED_BY_FILTER_OPTIONS.map((u) => <option key={u} value={u}>{u}</option>)}
        </select>
      </div>
      ...
      <div className="mt-5 flex items-center justify-between gap-2">
        <Button type="button" variant="ghost" onClick={onReset}>Clear</Button>
        <Button type="button" variant="primary" onClick={onApply}>Apply Filters</Button>
      </div>
    </div>
  );
}
