...
import { Eye, Download, MoreVertical, Star, Trash2, Share2, Pencil } from "lucide-react";
...
export function DocumentsTable({ documents, isLoading, page, pageSize, totalCount, onPageChange, onView, showUploadedBy = true }) {
  const columns = [
    ...
    { key: "category", header: "Category", render: (doc) => doc.category ?? "–" },
    ...(showUploadedBy ? [{ key: "uploadedBy", header: "Uploaded By", render: (doc) => {
      const style = avatarStyle(doc.uploadedByName);
      return (
    ...
    { key: "size", header: "File Size", render: (doc) => doc.size },
        ...
        <DropdownMenu.Root>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content align="end" sideOffset={6} className="z-50 w-48 rounded-md border border-border bg-white p-1.5 shadow-elevated animate-fade-in">
              <DropdownMenu.Item className={menuItemClass}><Star className="h-4 w-4" /> {doc.starred ? "Remove Star" : "Add to Starred"}</DropdownMenu.Item>
