...
import { useSharedDocumentDetail } from "@/hooks/useSharedDocumentDetail";
import { SharedDocumentsTable } from "@/components/document-management/SharedDocumentsTable";
import { SharedDocumentDetailsModal } from "@/components/document-management/SharedDocumentDetailsModal";
import { SharedDocumentsFiltersCard } from "@/components/document-management/SharedDocumentsFiltersCard";
...
import { SHARED_DOCUMENTS_QUICK_ACTIONS, SHARED_TYPE_FILTER_OPTIONS } from "@/lib/mock/vmSharedDocumentsMockData";
...
export function SharedDocumentsView() {
  ...
  const { document: doc, isLoading: isDetailLoading } = useSharedDocumentDetail(selectedId);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-ink">Shared Documents</h1>
        <p className="mt-1 text-sm text-ink-subtle">Documents that have been shared with you by other users.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_300px]">
        ...
        <div className="flex flex-col gap-6">
          <SharedDocumentsFiltersCard
            dateRange={dateRange} setDateRange={setDateRange}
            sharedByFilter={sharedByFilter} setSharedByFilter={setSharedByFilter}
          ...
          />
          <QuickActionsCard actions={SHARED_DOCUMENTS_QUICK_ACTIONS} />
          <SimpleStorageOverviewCard data={storageOverview} totalLabelText="Total Shared Storage:" />
        </div>
      </div>

      <SharedDocumentDetailsModal open={modalOpen} onOpenChange={setModalOpen} document={doc} isLoading={isDetailLoading} />
    </div>
  );
}
