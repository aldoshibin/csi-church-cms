...
import { TRANSFER_STEPS } from "@/hooks/useMemberTransferInForm";

export function TransferStepper({ currentStep, completedSteps, onStepClick }) {
  return (
      ...
      {TRANSFER_STEPS.map((step, idx) => {
        ...
        return (
          <React.Fragment key={step.key}>
            ...
            {idx < TRANSFER_STEPS.length - 1 && <span className="mx-4 h-px w-10 shrink-0 bg-border" />}
          </React.Fragment>
        );
      })}
    </div>
