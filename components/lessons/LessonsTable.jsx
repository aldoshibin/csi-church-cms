...
import { LESSON_STATUS_VARIANT, LESSON_CLASS_OPTIONS, LESSON_STATUS_OPTIONS } from "@/lib/mock/lessonsMockData";
import { formatDate } from "@/lib/utils";

export function LessonsTable({
  lessons, isLoading, pagination,
  search, onSearchChange, classFilter, onClassFilterChange, statusFilter, onStatusFilterChange,
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2.5">
          ...
          <Link href={`/sunday-school/lessons/${row.id}`} className="font-medium text-ink hover:text-interactive-600 hover:underline">
            {row.title}
          </Link>
        </div>
      ),
    },
    { key: "className", header: "Class", render: (row) => <span className="text-ink-muted">{row.className}</span> },
    ...
    { key: "date", header: "Date", render: (row) => <span className="text-ink-muted">{formatDate(row.date)}</span> },
    {
      ...
      render: (row) => <Badge variant={LESSON_STATUS_VARIANT[row.status] ?? "default"}>{row.status}</Badge>,
    },
    {
      key: "actions", header: "Actions", cellClassName: "text-right", className: "text-right",
      render: (row) => (
        <div className="flex items-center justify-end gap-1.5">
          <Link href={`/sunday-school/lessons/${row.id}`} className="flex h-8 w-8 items-center justify-center rounded-md border border-border text-ink-subtle hover:bg-surface-canvas" aria-label={`View ${row.title}`}>
          ...
          <LessonRowActionsMenu
            lesson={row}
            onView={() => {}} onEdit={onEdit} onDuplicate={onDuplicate} onPublish={onPublish} onArchive={onArchive} onDelete={onDelete}
          />
        </div>
      ),
    },
