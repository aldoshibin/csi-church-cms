...
export function ElectionOverviewCard({ data }) {
  if (!data) return null;
  return (
    <div className="rounded-lg border border-border bg-white p-5 shadow-card">
      <h3 className="text-sm font-semibold text-ink">Election Overview</h3>
      ...
      <p className="mt-4 flex items-center justify-between border-t border-border pt-3 text-sm">
        <span className="text-ink-subtle">Total Elections</span>
        <span className="font-medium text-ink">{data.total}</span>
      </p>
      <Link href="/election-management/elections">
        <Button type="button" variant="secondary" className="mt-3 w-full">View All Elections</Button>
      </Link>
    </div>
  );
}
