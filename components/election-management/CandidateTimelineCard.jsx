"use client";

import { Check, Clock } from "lucide-react";
import { formatDate } from "@/lib/utils";

const STATE_STYLE = {
  done: "bg-success-500 text-white",
  current: "bg-warning-500 text-white",
  pending: "bg-surface-muted text-ink-subtle",
};

export function CandidateTimelineCard({ timeline, title = "Timeline" }) {
  if (!timeline?.length) return null;
  return (
    <div className="rounded-lg border border-border bg-white p-5 shadow-card">
      <h3 className="text-sm font-semibold text-ink">{title}</h3>
      <div className="mt-4 flex flex-col">
        {timeline.map((step, idx) => (
          <div key={step.key} className="relative flex gap-3 pb-6 last:pb-0">
            {idx < timeline.length - 1 && (
              <span className="absolute left-[11px] top-6 h-[calc(100%-1.25rem)] w-px bg-border" />
            )}
            <span className={`z-10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${STATE_STYLE[step.state] ?? STATE_STYLE.pending}`}>
              {step.state === "done" ? <Check className="h-3.5 w-3.5" /> : <Clock className="h-3.5 w-3.5" />}
            </span>
            <div>
              <p className="text-sm font-semibold text-ink">{step.label}</p>
              <p className="text-xs text-ink-subtle">{formatDate(step.on, { hour: "numeric", minute: "2-digit" })}</p>
              <p className="text-xs text-ink-subtle">by {step.byName}</p>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
