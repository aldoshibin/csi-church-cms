...
import { VOTERS_LIST_MOCK, VOTERS_OVERVIEW_MOCK } from "@/lib/mock/vmVotersMockData";

export function useVoters() {
  const [voters, setVoters] = React.useState(VOTERS_LIST_MOCK);
  const [isLoading, setIsLoading] = React.useState(true);

  const [search, setSearch] = React.useState("");
  const [electionFilter, setElectionFilter] = React.useState("All Elections");
  const [statusFilter, setStatusFilter] = React.useState("All Status");
  const [membershipFilter, setMembershipFilter] = React.useState("All Memberships");
  ...
  const refetch = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const result = await electionManagementService.listVoters({
        search, electionFilter, statusFilter, membershipFilter, page, pageSize,
      });
      setVoters(result?.voters ?? VOTERS_LIST_MOCK);
    } catch {
      setVoters(VOTERS_LIST_MOCK);
    } finally {
      setIsLoading(false);
    }
  }, [search, electionFilter, statusFilter, membershipFilter, page, pageSize]);
  ...
  const clearFilters = () => {
    setSearch(""); setElectionFilter("All Elections"); setStatusFilter("All Status"); setMembershipFilter("All Memberships");
  };
  ...
  const filtered = React.useMemo(() => {
    return voters.filter((v) => {
      ...
      const matchesMembership = membershipFilter === "All Memberships" || v.membershipType === membershipFilter;
