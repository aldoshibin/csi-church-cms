...
import {
  PRAYER_REQUESTS_LIST_MOCK, PRAYER_STATS_MOCK, PRAYER_GROUPS_MOCK, UPCOMING_PRAYER_MEETINGS_MOCK,
} from "@/lib/mock/prayerRequestsMockData";

const PAGE_SIZE = 7;

export function usePrayerMinistryDashboard() {
  const [requests, setRequests] = React.useState(PRAYER_REQUESTS_LIST_MOCK);
  const [stats, setStats] = React.useState(PRAYER_STATS_MOCK);
  const [groups] = React.useState(PRAYER_GROUPS_MOCK);
  const [upcomingMeetings] = React.useState(UPCOMING_PRAYER_MEETINGS_MOCK);
  ...
  const refetch = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const result = await prayerMinistryService.getDashboard({ statusFilter, categoryFilter, groupFilter, page });
  ...
  return {
    requests: paged, totalCount: filtered.length, isLoading, stats, groups, upcomingMeetings,
    ...
    resetFilters: () => { setStatusFilter("All Status"); setCategoryFilter("All Categories"); setGroupFilter("All Groups"); },
  };
}
