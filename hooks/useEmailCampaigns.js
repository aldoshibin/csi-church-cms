...
import {
  CAMPAIGNS_LIST_MOCK, CAMPAIGN_OVERVIEW_STATS_MOCK, CAMPAIGN_OVERVIEW_DONUT_MOCK, CAMPAIGN_TYPES_SIDEBAR_MOCK,
...
export function useEmailCampaigns() {
  const [campaigns, setCampaigns] = React.useState(CAMPAIGNS_LIST_MOCK);
  const [stats] = React.useState(CAMPAIGN_OVERVIEW_STATS_MOCK);
  const [donut] = React.useState(CAMPAIGN_OVERVIEW_DONUT_MOCK);
  const [campaignTypes] = React.useState(CAMPAIGN_TYPES_SIDEBAR_MOCK);
  ...
  const refetch = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const result = await emailCampaignsService.listCampaigns({ activeTab, search, statusFilter, typeFilter, page });
      setCampaigns(result?.campaigns ?? CAMPAIGNS_LIST_MOCK);
    } catch {
      setCampaigns(CAMPAIGNS_LIST_MOCK);
    } finally {
      setIsLoading(false);
    }
