...
import { ChevronRight, ArrowLeft, ChevronLeft as ChevronLeftIcon, ArrowRight, Check,Calendar } from "lucide-react";
import { Button } from "@/components/ui/Button";
import { FamilyStepRail } from "@/components/family-management/add/FamilyEventStepRail";
import { EventSummaryPanel } from "@/components/family-management/add/FamilyEventSummaryPanel";
import { StepFamilyDetails } from "@/components/family-management/add/StepFamilyEventDetails";
import { StepFamilyDateandTime } from "@/components/family-management/add/StepFamilyEventDateandTimeDetails";
import { StepVenueOrganizer } from "@/components/family-management/add/StepEventVenu";
import { StepAttendees } from "@/components/family-management/add/StepEventAttendance";
import { StepReviewConfirm } from "@/components/family-management/add/StepEventReviewConfirm";
import { StepSuccess } from "@/components/family-management/add/StepEventCreated";
...
export default function AddNewFamilyPage() {
  ...
  const stepBody = {
    ...
    2: <StepFamilyDateandTime form={form} set={set} />,
    3: <StepVenueOrganizer form={form} set={set}/>,
    4: <StepAttendees form={form} set={set} />,
    5: <StepReviewConfirm form={form} members={members} />,
    6: <StepSuccess form={form} members={members} />,
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 flex items-center gap-1.5 text-sm">
            ...
            <Link href="/families/events" className="text-interactive-500 hover:underline">Family Event</Link>
            <ChevronRight className="h-3.5 w-3.5 text-ink-subtle" />
        ...
        <button
          type="button"
          onClick={() => router.push("/families/events")}
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_340px]">
        <div>
          ...
          <div className="mt-5 flex items-center justify-between">
            ...
            {step !== 6 && (
            step === 5 ? (
              <Button type="button" leftIcon={<Calendar className="h-4 w-4" />} onClick={goNext}>
                Create Event
              </Button>
            ) : (
        ...
        <EventSummaryPanel step={step} form={form}  />
      </div>

      {/* Sticky page-level footer bar — separate from the in-card Previous/Next above,
          matches the persistent Cancel/Save & Next bar visible on every step  */}
