...
import { UpcomingMilestonesTab } from "@/components/senior-registry/tabs/UpcomingMilestonesTab";
import { HealthAssistanceTab } from "@/components/senior-registry/tabs/HealthAssistanceTab";
import { FollowUpNotesTab } from "@/components/senior-registry/tabs/FollowUpNotesTab";
import { AddMilestoneModal } from "@/components/senior-registry/AddMilestoneModal";
import { AddAssistanceModal } from "@/components/senior-registry/AddAssistanceModal";
import { AddFollowUpEntryModal } from "@/components/senior-registry/AddFollowUpEntryModal";
...
export default function SeniorCitizenRegistryPage() {
  ...
  const [assistanceModalOpen, setAssistanceModalOpen] = React.useState(false);
  const [followUpEntryModalOpen, setFollowUpEntryModalOpen] = React.useState(false);
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="min-w-0 rounded-lg border border-border bg-white shadow-card">
            ...
            {activeTab === "list" ? (
              ...
              <UpcomingMilestonesTab
                ...
                onView={(item) => toast({ variant: "info", title: item.title, description: item.subtitle })}
              />
            ) : activeTab === "health" ? (
              <HealthAssistanceTab onAddAssistance={() => setAssistanceModalOpen(true)} />
            ) : (
              <FollowUpNotesTab
                onAddNote={() => setFollowUpEntryModalOpen(true)}
                onLoadMore={() => toast({ variant: "info", title: "That's all the sample data", description: "This tab has no real notes-feed endpoint yet — see the README." })}
              />
            )}
          </div>
        </div>
      ...
      <AddMilestoneModal open={milestoneModalOpen} onOpenChange={setMilestoneModalOpen} />
      <AddAssistanceModal open={assistanceModalOpen} onOpenChange={setAssistanceModalOpen} />
      <AddFollowUpEntryModal open={followUpEntryModalOpen} onOpenChange={setFollowUpEntryModalOpen} />
    </div>
  );
}
